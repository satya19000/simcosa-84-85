...
import type { Decision, DecisionProvider } from './DecisionTypes'
...
class DefaultDecisionProvider implements DecisionProvider {
  readonly name = 'default'

  async generate(context: ContextSnapshot, _memory: MemoryBlock[]): Promise<Decision[]> {
    ...
    const highDueToday = context.dueTodayTasks.filter(
    ...
    if (
      context.allPendingTasks.length > 8 &&
    ...
    if (context.recentContacts.length > 0 && context.overdueTasks.length === 0) {
      const lastInteracted = context.recentContacts.find(
        (c) => c.relationshipNotes && c.relationshipNotes.length > 10
      )
      if (lastInteracted) {
        decisions.push({
...
export async function generateDecisions(
  ...
  const results = await Promise.all(
    decisionProviders.map((p) => p.generate(context, memory).catch(() => [] as Decision[]))
  )

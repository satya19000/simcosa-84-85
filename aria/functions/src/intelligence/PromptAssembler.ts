...
function buildContextSection(ctx: ContextSnapshot): string {
  const lines: string[] = [
    `Date: ${ctx.dateFull}`,
...
export function assemblePrompt(
  ...
  const sectionsIncluded: string[] = ['system', 'context']
  ...
  const memorySection = memoryParts.length > 0
    ? `=== MEMORY ===\n${memoryParts.join('\n\n')}`
    : ''
  if (memoryParts.length > 0) sectionsIncluded.push('memory')
  ...
  if (decisionsSection) parts.push(decisionsSection)

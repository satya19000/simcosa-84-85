...
import type { MemoryBlock, MemoryProvider } from './MemoryTypes'
import type { ContextSnapshot } from './ContextTypes'
import type { EngineConfig } from './EngineConfig'
import { rankTasks, rankContacts } from './PriorityEngine'
...
function getCached(key: string, ttlMs: number): MemoryBlock[] | null {
...
function setCached(key: string, blocks: MemoryBlock[]): void {
...
function buildTasksBlock(context: ContextSnapshot): MemoryBlock {
  const nowIso = new Date().toISOString()
  const ranked = rankTasks(context.allPendingTasks, nowIso).slice(0, 10)
  ...
  const otherHighPriority = ranked.filter(
    (t) => !context.overdueTasks.find((o) => o.id === t.id) && !context.dueTodayTasks.find((d) => d.id === t.id)
  ).filter((t) => t.priority === 'high' || t.priority === 'critical').slice(0, 3)
...
function buildRemindersBlock(context: ContextSnapshot): MemoryBlock {
  const lines: string[] = []
...
function buildContactsBlock(context: ContextSnapshot, messageLower: string): MemoryBlock {
  const ranked = rankContacts(context.recentContacts, messageLower)
...
function buildBriefingBlock(context: ContextSnapshot): MemoryBlock | null {
  if (!context.latestBriefing) return null
...
export async function buildMemory(
  ...
  message: string,
  config: EngineConfig
): Promise<{ blocks: MemoryBlock[]; cacheHits: number; cacheMisses: number }> {
  const messageLower = message.toLowerCase()
  let cacheHits = 0
  let cacheMisses = 0

  // Collect blocks from registered external providers (parallelized + cached)
  const externalBlockArrays = await Promise.all(
    memoryProviders.map(async (provider) => {
  ...
  const builtIn: MemoryBlock[] = []

  const tasksBlock = buildTasksBlock(context)
  if (context.allPendingTasks.length > 0) builtIn.push(tasksBlock)

  const remindersBlock = buildRemindersBlock(context)
  if (context.allPendingReminders.length > 0) builtIn.push(remindersBlock)

  const contactsBlock = buildContactsBlock(context, messageLower)
  if (context.recentContacts.length > 0) builtIn.push(contactsBlock)

  const briefingBlock = buildBriefingBlock(context)
  if (briefingBlock) builtIn.push(briefingBlock)

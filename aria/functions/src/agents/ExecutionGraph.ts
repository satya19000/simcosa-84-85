...
import type { AgentTaskId, GraphRunId, AgentTaskStatus } from './AgentTypes'
...
export class ExecutionGraph {
  private nodes = new Map<AgentTaskId, GraphNode>()

  constructor(readonly graphRunId: GraphRunId) {}

  addTask(task: AgentTask): void {
    this.nodes.set(task.taskId, { task })
  }

  setResult(taskId: AgentTaskId, result: AgentResult): void {
  ...
  markStarted(taskId: AgentTaskId): void {
    const node = this.nodes.get(taskId)
    if (node) {
  ...
  setStatus(taskId: AgentTaskId, status: AgentTaskStatus): void {
    const node = this.nodes.get(taskId)
    if (node) node.task.status = status
  }

  getNode(taskId: AgentTaskId): GraphNode | undefined {
  ...
  getReady(): AgentTask[] {
    return Array.from(this.nodes.values())
      .filter((node) => {
        if (node.task.status !== 'pending' && node.task.status !== 'queued') return false
  ...
  isComplete(): boolean {
    return Array.from(this.nodes.values()).every((n) =>
      ['completed', 'failed', 'skipped', 'cancelled'].includes(n.task.status)
    )
  }
  ...
  topologicalOrder(): AgentTaskId[] {
    const inDegree = new Map<AgentTaskId, number>()
    const adjacency = new Map<AgentTaskId, AgentTaskId[]>()

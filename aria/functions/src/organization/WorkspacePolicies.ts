...
import type { MemberRecord } from './WorkspaceTypes'
...
export class WorkspacePolicies {
  ...
  canInviteMore(currentMemberCount: number): boolean {
    return currentMemberCount < this.config.maxMembersPerOrganization
  }

  canCreateMoreWorkspaces(currentWorkspaceCount: number): boolean {
    return currentWorkspaceCount < this.config.maxWorkspacesPerOrganization
  }

  invitationExpiresAt(now: Date = new Date()): string {
    return new Date(now.getTime() + this.config.invitationExpiryMs).toISOString()
  }

  isInvitationExpired(expiresAt: string, now: Date = new Date()): boolean {
    return new Date(expiresAt).getTime() < now.getTime()
  }

  /** Only Owner/Admin may manage org-level settings; Manager may manage their own workspace. */
  canManageOrganization(member: MemberRecord): boolean {
    return member.role === 'owner' || member.role === 'admin'
  }

  canManageWorkspace(member: MemberRecord, workspaceId: string): boolean {
    if (this.canManageOrganization(member)) return true

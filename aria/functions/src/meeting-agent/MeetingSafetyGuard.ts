...
export class MeetingSafetyError extends Error {
  constructor(
...
export class MeetingSafetyGuard {
  /**
  ...
  assertRecordingStartSafe(options: {
    ...
    if (options.stealthMode) {
      throw new MeetingSafetyError(
        'STEALTH_RECORDING_BLOCKED',
        'Stealth recording is unconditionally blocked. ARIA will never record audio or capture transcripts without the user\'s explicit knowledge and visible session indicator.'
      )
    }
    ...
    if (options.hiddenMicrophone) {
      throw new MeetingSafetyError(
        'HIDDEN_MICROPHONE_BLOCKED',
        'Hidden microphone access is unconditionally blocked. Microphone use must be visible to the user at all times.'
      )
    }
    ...
    if (options.backgroundListening && !options.hasVisibleSession) {
      throw new MeetingSafetyError(
        'BACKGROUND_LISTENING_BLOCKED',
        'Background listening without a visible active session is unconditionally blocked. A visible session indicator must be shown to the user whenever audio is being processed.'
      )
    }
    ...
    if (!options.userConsentGranted) {
      throw new MeetingSafetyError(
        ...
        'Recording cannot start without explicit user consent. The user must actively enable recording.'
      )
    }
  }
   ...
   */
  assertNoAutoSend(intent: string): void {
    ...
    for (const { pattern, code } of autoSendPatterns) {
      if (lower.includes(pattern)) {
        throw new MeetingSafetyError(
          code,
          `Auto-send of communications is unconditionally blocked. All outgoing messages require explicit approval via MeetingApprovalBridge. Detected pattern: "${pattern}".`
        )
      }
    }
  }
   ...
   */
  assertNoAutoTaskCreation(intent: string): void {
    const lower = intent.toLowerCase()
    if (lower.includes('auto-create task') || lower.includes('automatically create reminder')) {
      throw new MeetingSafetyError(
        'AUTO_TASK_CREATION_BLOCKED',
        'Automatic task/reminder creation is unconditionally blocked. Tasks and reminders are suggestions only and require explicit user approval before creation.'
      )
    }
  }
  ...
  assertCallRecordingAuthorized(options: {
    ...
    requiresParticipantConsent: boolean
  }): void {
    if (!options.userConsentGranted) {
      throw new MeetingSafetyError(
        'UNAUTHORIZED_CALL_RECORDING',
        'Call recording requires explicit user authorization. Unauthorized call recording is unconditionally blocked.'
      )
    }
    if (options.requiresParticipantConsent && !options.participantsNotified) {
      throw new MeetingSafetyError(
        'PARTICIPANT_CONSENT_REQUIRED',
        'Recording this call type requires participant notification/consent. Proceeding without notifying participants is blocked by policy.'
      )
    }
  }
   ...
   */
  assertNoStealthCallJoin(options: { stealthJoin?: boolean }): void {
    if (options.stealthJoin) {
      throw new MeetingSafetyError(
        'STEALTH_CALL_JOIN_BLOCKED',
        'Stealth call joining is unconditionally blocked. ARIA will never join a meeting or call without a visible indication to the user.'
      )
    }
  }
}

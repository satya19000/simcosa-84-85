import { MARKETPLACE_CATEGORIES, MARKETPLACE_ITEM_TYPES, SKILL_PERMISSION_SCOPES } from './MarketplaceTypes'
import type { SkillManifest } from './MarketplaceTypes'
...
export class SkillValidator {
  static validateManifest(manifest: Partial<SkillManifest>): void {
    ...
    if (!manifest.category || !MARKETPLACE_CATEGORIES.includes(manifest.category)) {
      throw new SkillValidationError('category', `must be one of: ${MARKETPLACE_CATEGORIES.join(', ')}`)
    }
    if (!manifest.itemType || !MARKETPLACE_ITEM_TYPES.includes(manifest.itemType)) {
      throw new SkillValidationError('itemType', `must be one of: ${MARKETPLACE_ITEM_TYPES.join(', ')}`)
    }
    if (!Array.isArray(manifest.permissions) || manifest.permissions.some((p) => !SKILL_PERMISSION_SCOPES.includes(p))) {
      throw new SkillValidationError('permissions', `must be an array of valid permission scopes: ${SKILL_PERMISSION_SCOPES.join(', ')}`)
    }
  ...
  static validateReview(input: { rating: number; reviewText: string }): void {

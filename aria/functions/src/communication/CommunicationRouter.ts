...
export class CommunicationRouter {
  ...
  constructor(
    ...
    apiKey: string,
    private readonly analysisBudget: number,
    private readonly summaryBudget: number,
    private readonly replyBudget: number
  ) {
...
Return ONLY valid JSON.`,
      ...
      if (!match) return this.emptyAnalysis(thread.id, thread.userId)

      const parsed = JSON.parse(match[0])
      const detectedItems: DetectedItem[] = parsed.detectedItems ?? []
...
Return ONLY valid JSON.`,
      ...
      const raw = block?.type === 'text' ? block.text : '{}'
      const match = raw.match(/\{[\s\S]*\}/)
      if (!match) return this.emptySummary(thread.id, thread.userId, type)
      const parsed = JSON.parse(match[0])
...
Return ONLY the reply text, no explanations.`,
  ...
  private buildSuggestions(threadId: string, items: DetectedItem[]): ConversationSuggestion[] {
  ...
  private emptyAnalysis(threadId: string, userId: string): ThreadAnalysis {
    return {
      id: uuidv4(),
      threadId,
      userId,
  ...
  private emptySummary(threadId: string, userId: string, type: SummaryType): ConversationSummary {
    return {
      id: uuidv4(),
      threadId,
      userId,

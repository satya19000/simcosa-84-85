 ...
 */
export class ComputerExecutionPipeline {
  constructor(
    ...
    private readonly validator: ComputerExecutionValidator
  ) {}
   ...
   */
  async execute(input: PipelineExecuteInput): Promise<PipelineExecutionResult> {
  ...
  private validateInputShape(input: PipelineExecuteInput): Record<string, unknown> {
    if (!input.tenantId?.trim()) throw new Error('tenantId is required.')
    if (!input.userId?.trim()) throw new Error('userId is required.')
    if (!input.plan?.planId) throw new Error('A valid plan with planId is required.')
    if (input.step === undefined || input.step === null) throw new Error('step is required.')
    if (!input.step.capabilityId) throw new Error('step.capabilityId is required.')
  ...
  private async runStage<T>(
    ...
    try {
      ...
      return { stage, success: true, durationMs: Date.now() - start, output: output as unknown }
    } catch (err) {
      return {
        stage,
        success: false,
  ...
  private skippedStage(stage: PipelineStageId, reason: string): PipelineStageResult {
  ...
  private buildResult(
    ...
    return {
      ...
      overallSuccess,
      stages,
      actionResult: execStage?.output as ReturnType<typeof Object.assign> | undefined,
      auditEventId: (auditStage?.output as { auditEventId?: string })?.auditEventId,

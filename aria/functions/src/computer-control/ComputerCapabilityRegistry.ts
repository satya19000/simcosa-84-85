import type { ComputerCapabilityDescriptor, ComputerCapabilityId } from './ComputerTypes'
 ...
 */
const CAPABILITIES: ComputerCapabilityDescriptor[] = [
  {
    id: 'readVisiblePage',
...
export class ComputerCapabilityRegistry {
  private readonly map = new Map<ComputerCapabilityId, ComputerCapabilityDescriptor>(
    CAPABILITIES.map((c) => [c.id, c])
  )

  getAll(): ComputerCapabilityDescriptor[] {
    return [...this.map.values()]
  }

  get(id: ComputerCapabilityId): ComputerCapabilityDescriptor | null {
    return this.map.get(id) ?? null
  }

  listAvailable(): ComputerCapabilityDescriptor[] {
    return this.getAll().filter((c) => !c.alwaysBlocked)
  }

  isAlwaysBlocked(id: ComputerCapabilityId): boolean {
    return this.map.get(id)?.alwaysBlocked === true
  }

  isPolicyBlocked(id: ComputerCapabilityId): boolean {
    return this.map.get(id)?.policyBlocked === true
  }

  requiresApproval(id: ComputerCapabilityId): boolean {

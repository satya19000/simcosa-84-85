...
const DOC_COL = (tenantId: string) => `tenants/${tenantId}/computerDocuments`
 ...
 */
export class ComputerDocumentBridge {
   ...
   */
  fileSelectedByUser(fileName: string, fileType: string, fileSizeBytes: number): FileMetadata {
    return {
      fileName,
      fileType,
      fileSizeBytes,
   ...
   */
  extractFileMetadata(req: DocumentAnalysisRequest): FileMetadata {
    return {
   ...
   */
  async handoffToDocumentIntelligence(
    req: DocumentAnalysisRequest,
    aiSummary?: string,
    aiActionItems?: ExtractedActionItem[]
  ): Promise<DocumentBridgeResult> {
    ...
    const result: DocumentBridgeResult = {
      documentId,
   ...
   */
  extractActionItems(aiItems?: ExtractedActionItem[]): ExtractedActionItem[] {
    return aiItems ?? []
  }
  ...
  suggestDocumentToTask(
    ...
    return actionItems
      ...
      .map((item) => ({
        ...
        _requiresUserApproval: true as const,
      }))
  }
  ...
  private generateStructuralSummary(
    fileName: string,
    fileType: string,
    fileSizeBytes: number,
    textLength: number
  ...
  private labelForMimeType(mimeType: string): string {

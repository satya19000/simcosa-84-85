...
import type { ComputerCapabilityId, ComputerPermission } from './ComputerTypes'
...
export class ComputerPermissions {
   ...
   * type in this phase.
   */
  private mapToPermissionAction(permission: ComputerPermission): 'security.manage' | 'tasks.write' | 'documents.write' {
    const highPriv: ComputerPermission[] = [
      'computer.screenshot',
      'computer.ocr',
    ...
    const docPriv: ComputerPermission[] = [
      'computer.files.read',
      'computer.files.write',
      'computer.browser.tabs',
      'computer.apps.open',
    ]
    if (highPriv.includes(permission)) return 'security.manage'
    if (docPriv.includes(permission)) return 'documents.write'
    return 'tasks.write'
  }

  /** Non-throwing check. Returns true only if all required permissions are granted. */
  async canExecuteCapability(tenantId: string, userId: string, capabilityId: ComputerCapabilityId): Promise<boolean> {
    const descriptor = this.capabilityRegistry.get(capabilityId)
    if (!descriptor) return false
    if (descriptor.alwaysBlocked) return false
    if (descriptor.policyBlocked) return false
  ...
  async requireCapabilityPermission(tenantId: string, userId: string, capabilityId: ComputerCapabilityId): Promise<void> {
    const descriptor = this.capabilityRegistry.get(capabilityId)
    if (!descriptor) throw new Error(`Unknown capability: ${capabilityId}`)
    if (descriptor.alwaysBlocked) throw new Error(`Capability "${capabilityId}" is unconditionally blocked.`)
    if (descriptor.policyBlocked) throw new Error(`Capability "${capabilityId}" is blocked by policy.`)
  ...
  async requireTenantMembership(tenantId: string, userId: string): Promise<void> {

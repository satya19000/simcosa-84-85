...
export interface TemporalContext {
  /** ISO-8601 current datetime in the user's timezone, e.g. 2026-06-29T14:30:00+05:30 */
  currentDatetimeISO: string
  /** IANA timezone, e.g. Asia/Kolkata */
  timezone: string
  /** Human-readable day name, e.g. "Sunday 29 June 2026" */
  todayLabel: string
  /** ISO date of tomorrow, e.g. 2026-06-30 */
 ...
 */
export function buildTemporalContext(userTimezone?: string): TemporalContext {
 ...
 */
export function validateISODatetime(value: string): string | null {
  if (!value || typeof value !== 'string') return 'datetime is missing'

  const parsed = Date.parse(value)
  if (isNaN(parsed)) return `"${value}" is not a valid datetime`
 ...
 */
export function buildTemporalSystemSection(ctx: TemporalContext): string {
  return `

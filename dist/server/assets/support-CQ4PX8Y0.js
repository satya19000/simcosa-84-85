...
const listMySupport = createServerFn({
  method: "GET"
}).middleware([requireAuth]).handler(listMySupport_createServerFn_handler, async ({
  context
}) => {
...
const createSupport = createServerFn({
  method: "POST"
}).middleware([requireAuth]).inputValidator((d) => d).handler(createSupport_createServerFn_handler, async ({
  data,
  context
}) => {

...
import { s as supabase } from "./router-CIkP24eF.js";
import { Search, Briefcase, MapPin, Mail, Phone, MessageCircle } from "lucide-react";
  ...
  const {
    ...
    queryFn: async () => {
      const {
        data: data2,
        error
      } = await supabase.from("profiles").select("*").eq("approved", true).order("full_name");

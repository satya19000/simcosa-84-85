  ...
  const {
    ...
    queryFn: async () => {
      const {
        data,
        error
      } = await supabase.from("gallery_items").select("*").order("created_at", {
        ascending: false
      });
...
function GalleryItem({
  ...
  useEffect(() => {
    supabase.storage.from("gallery").createSignedUrl(item.storage_path, 3600).then(({
      data
  ...
  return /* @__PURE__ */ jsxs("div", { className: "rounded-2xl overflow-hidden shadow-sm border border-amber-100 hover:shadow-md transition-shadow group", children: [
    /* @__PURE__ */ jsxs("div", { className: `${item.media_type === "video" ? "aspect-video" : "aspect-square"} bg-amber-50 flex items-center justify-center`, children: [
      url && (item.media_type === "video" ? /* @__PURE__ */ jsx("video", { src: url, controls: true, className: "w-full h-full object-cover" }) : /* @__PURE__ */ jsx("img", { src: url, alt: item.caption ?? "Photo", className: "w-full h-full object-cover group-hover:scale-105 transition-transform duration-300", loading: "lazy" })),
      !url && /* @__PURE__ */ jsx("div", { className: "animate-pulse bg-amber-100 w-full h-full" })
    ] }),

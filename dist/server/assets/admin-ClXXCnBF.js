import { c as createServerRpc } from "./createServerRpc-B-vKRJ4e.js";
import { a as requireAdmin } from "./middleware-BymfKMq5.js";
import { q as query, P as PROFILE_COLUMNS, w as withTransaction } from "../server.js";
import { c as createServerFn } from "./server-B3JlQD99.js";
...
const adminListEvents = createServerFn({
  ...
  const res = await query(`SELECT ${ADMIN_EVENT_COLUMNS}
       FROM events
...
const adminCreateEvent = createServerFn({
  ...
  const isPublished = data.is_published !== void 0 ? data.is_published : true;
  const res = await query(`INSERT INTO events
       ...
       RETURNING id`, [title, description || null, location || null, eventDate, data.end_date || null, coverUrl, fbStoragePath, fileName, fileSize, coverMime, eventType, data.rsvp_enabled ?? false, data.external_link || null, isPublished, context.userId]);
  return {
    ok: true,
    id: res.rows[0].id
  };
});
const adminEditEvent_createServerFn_handler = createServerRpc({
  id: "31bde2814aa706dc87e59e5796eb96296cd1b9b8cd53b725218efc27c9ce74f9",
  name: "adminEditEvent",
  filename: "src/api/admin.ts"
}, (opts) => adminEditEvent.__executeServer(opts));
const adminEditEvent = createServerFn({
  method: "POST"
}).middleware([requireAdmin]).inputValidator((d) => d).handler(adminEditEvent_createServerFn_handler, async ({
  data
}) => {
  const title = data.title.trim();
  const description = (data.description ?? "").trim();
  ...
  if (data.url) {
    ...
    await query(`UPDATE events SET title=$2, description=$3, location=$4, event_date=$5, end_date=$6,
           ...
           updated_at=now()
         WHERE id=$1`, [data.id, title, description || null, location || null, data.event_date, data.end_date || null, data.event_type || "upcoming", data.rsvp_enabled ?? false, data.external_link || null, isPublished, data.url, data.storagePath || null, data.fileName || null, data.fileSize ?? null, data.mimeType || null]);
  } else {
    await query(`UPDATE events SET title=$2, description=$3, location=$4, event_date=$5, end_date=$6,
           event_type=$7, rsvp_enabled=$8, external_link=$9, is_published=$10, updated_at=now()
         WHERE id=$1`, [data.id, title, description || null, location || null, data.event_date, data.end_date || null, data.event_type || "upcoming", data.rsvp_enabled ?? false, data.external_link || null, isPublished]);
  }
  return {
    ok: true
  };
...
const adminToggleEventPublished_createServerFn_handler = createServerRpc({
  id: "43bb1b2eb266a0be6a5cffc68cd272e450fee8a245b3fb23726d463417f5b9fa",
  name: "adminToggleEventPublished",
  filename: "src/api/admin.ts"
}, (opts) => adminToggleEventPublished.__executeServer(opts));
const adminToggleEventPublished = createServerFn({
  method: "POST"
}).middleware([requireAdmin]).inputValidator((d) => d).handler(adminToggleEventPublished_createServerFn_handler, async ({
  data
}) => {
...
const adminReorderGallery_createServerFn_handler = createServerRpc({
  id: "a553de435f7206474ae1eee729506b39ba7c4d923a1be2069e453139db5bc68d",
  name: "adminReorderGallery",
  filename: "src/api/admin.ts"
}, (opts) => adminReorderGallery.__executeServer(opts));
const adminReorderGallery = createServerFn({
  method: "POST"
}).middleware([requireAdmin]).inputValidator((d) => d).handler(adminReorderGallery_createServerFn_handler, async ({
  data
}) => {
  const existing = await query(`SELECT id FROM gallery_items WHERE deleted_at IS NULL`);
  ...
  await Promise.all(data.orderedItemIds.map((id, i) => query(`UPDATE gallery_items SET sort_order = $1 WHERE id = $2`, [i, id])));
  return {
    ok: true
  };
});
...
const adminListMemories = createServerFn({
  ...
  const res = await query(`SELECT m.id, m.title, m.body, m.author_name,
...
const adminEditMemoryAuthor_createServerFn_handler = createServerRpc({
  id: "868909a35faf0ae1a8048570274ae302287533126b7d78e38915e01be0eaa980",
  name: "adminEditMemoryAuthor",
  filename: "src/api/admin.ts"
}, (opts) => adminEditMemoryAuthor.__executeServer(opts));
const adminEditMemoryAuthor = createServerFn({
  method: "POST"
}).middleware([requireAdmin]).inputValidator((d) => d).handler(adminEditMemoryAuthor_createServerFn_handler, async ({
  data
}) => {
...
const adminFindDuplicateMemories_createServerFn_handler = createServerRpc({
  id: "94c610dc6dc8a18b0614c96496a75b481dbd4fcbc7615ea6ddffd12ac6c0d884",
  name: "adminFindDuplicateMemories",
  filename: "src/api/admin.ts"
}, (opts) => adminFindDuplicateMemories.__executeServer(opts));
const adminFindDuplicateMemories = createServerFn({
  method: "GET"
}).middleware([requireAdmin]).handler(adminFindDuplicateMemories_createServerFn_handler, async () => {
  const res = await query(`SELECT m.title, m.body, m.user_id, p.full_name, m.id, m.created_at,
  ...
  for (const row of res.rows) {
    const sameKey = current && current.user_id === row.user_id && current.title === row.title && current.body === row.body;
    const withinWindow = sameKey && lastCreatedAt !== null && new Date(row.created_at).getTime() - lastCreatedAt <= 10 * 60 * 1e3;
...
const adminMergeMemories_createServerFn_handler = createServerRpc({
  id: "bfcff4d5245e6629dcb6468f93e302b08157b47db246d579fb46f1ce43c9d241",
  name: "adminMergeMemories",
  filename: "src/api/admin.ts"
}, (opts) => adminMergeMemories.__executeServer(opts));
const adminMergeMemories = createServerFn({
  method: "POST"
}).middleware([requireAdmin]).inputValidator((d) => d).handler(adminMergeMemories_createServerFn_handler, async ({
  data
}) => {
  if (!data.duplicateIds.length) return {
    ok: true
  };
  await withTransaction(async (tx) => {
    for (const dupId of data.duplicateIds) {
      if (dupId === data.keepId) continue;
      const legacy = await tx.query(`SELECT image_url, fb_storage_path, file_name, file_size FROM memories WHERE id = $1`, [dupId]);
      const dupRow = legacy.rows[0];
      if (dupRow?.image_url) {
        await tx.query(`INSERT INTO memory_images (memory_id, image_url, fb_storage_path, file_name, file_size)
             VALUES ($1, $2, $3, $4, $5)`, [data.keepId, dupRow.image_url, dupRow.fb_storage_path, dupRow.file_name, dupRow.file_size]);
      }
...
export {
  ...
  adminEditEvent_createServerFn_handler,
  adminEditMember_createServerFn_handler,
  adminEditMemoryAuthor_createServerFn_handler,
  adminFindDuplicateMemories_createServerFn_handler,

...
import { Loader2, ArrowLeft, Briefcase, MapPin, Mail, Phone, MessageCircle, Star, Heart, Users, Smile, BookOpen, Camera, Film, FileText, HardDrive, AlertTriangle, Plus, Eye, EyeOff, Check, Pencil, Trash2 } from "lucide-react";
import { g as getMemberBySlug, a as getMemberStorageUsage, l as listMemberBlogItems, d as deleteMemberBlogItem, b as addMemberBlogItem, e as editMemberBlogItem } from "./memberBlogs-Ca4M-RnL.js";
import { R as Route, u as useAuth, M as MEMBER_QUOTA_BYTES, V as VIDEO_MAX_BYTES } from "./router-TV6HlS94.js";
import { I as ImageLightbox } from "./ImageLightbox-Dt94sEvv.js";
import { u as useUploadQueue, D as DropzoneUpload, d as deleteFromFirebaseStorage, c as compressImage, a as uploadToFirebaseStorageResumable } from "./useUploadQueue-BrQcAYe7.js";
...
function validateFileSize(file, tab) {
...
function StorageBar({
  ...
  return /* @__PURE__ */ jsxs("div", { className: "bg-white rounded-2xl border border-amber-100 shadow-sm p-4 flex flex-col gap-2", children: [
    /* @__PURE__ */ jsxs("div", { className: "flex items-center gap-2", children: [
      /* @__PURE__ */ jsx(HardDrive, { className: "h-4 w-4 text-amber-400 shrink-0" }),
      /* @__PURE__ */ jsxs("span", { className: "text-xs font-semibold text-gray-600", children: [
        "Storage used: ",
        fmtBytes(usage.total_bytes),
        " / ",
        fmtBytes(MEMBER_QUOTA_BYTES)
      ] }),
      atLimit && /* @__PURE__ */ jsx(AlertTriangle, { className: "h-4 w-4 text-red-500 shrink-0 ml-auto" })
    ] }),
    /* @__PURE__ */ jsx("div", { className: "w-full h-2 rounded-full bg-amber-50 overflow-hidden", children: /* @__PURE__ */ jsx("div", { className: `h-full rounded-full transition-all duration-300 ${atLimit ? "bg-red-500" : nearLimit ? "bg-amber-500" : "bg-emerald-500"}`, style: {
      width: `${pct}%`
    } }) }),
    /* @__PURE__ */ jsxs("div", { className: "flex flex-wrap gap-3 text-xs text-gray-400", children: [
      /* @__PURE__ */ jsxs("span", { children: [
        usage.photo_count,
        " photo",
        usage.photo_count !== 1 ? "s" : ""
      ] }),
      /* @__PURE__ */ jsx("span", { children: "·" }),
      /* @__PURE__ */ jsxs("span", { children: [
        usage.video_count,
        " video",
        usage.video_count !== 1 ? "s" : ""
      ] }),
      /* @__PURE__ */ jsx("span", { children: "·" }),
      /* @__PURE__ */ jsxs("span", { children: [
        usage.doc_count,
...
function ItemForm({
  ...
  const handleFilesChange = (incoming) => {
    ...
    const err = validateFileSize(f[0]);
    if (err) {
      toast.error(err);
      return;
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      qc.invalidateQueries({
        queryKey: ["member-blog-usage", memberId]
      });
      onDone();
    } catch (err) {
  ...
  return /* @__PURE__ */ jsxs("form", { onSubmit: handleSubmit, className: "space-y-4 bg-white rounded-2xl border border-amber-100 shadow-sm p-5", children: [
    ...
    tabAllowsFile(tab) && !existing?.file_url && /* @__PURE__ */ jsxs("div", { className: "space-y-2", children: [
      /* @__PURE__ */ jsx(DropzoneUpload, { files, onFilesChange: handleFilesChange, accept: tabAccept(tab), multiple: false, disabled: saving, progress: uploadQueue.progress, label: tab === "photos" ? "Upload a photo (max 10 MB — converted to WebP)" : tab === "videos" ? "Upload a video (max 25 MB)" : tab === "files" ? "Upload a file — PDF, Word, etc. (max 10 MB)" : "Attach a photo (optional, max 10 MB)" }),
      tab === "videos" && /* @__PURE__ */ jsxs("p", { className: "text-xs text-amber-600 flex items-center gap-1.5", children: [
...
function ItemCard({
  ...
  return /* @__PURE__ */ jsxs("div", { className: `bg-white rounded-2xl border border-amber-100 shadow-sm overflow-hidden ${!item.is_published ? "opacity-60" : ""}`, children: [
    ...
    item.file_url && !isImage && !isVideo && /* @__PURE__ */ jsxs("a", { href: item.file_url, target: "_blank", rel: "noreferrer", className: "flex items-center gap-3 px-4 py-3 bg-amber-50 border-b border-amber-100 hover:bg-amber-100 transition-colors", children: [
      ...
      item.file_size && /* @__PURE__ */ jsx("span", { className: "ml-auto text-xs text-gray-400 shrink-0", children: fmtMB(item.file_size) })
    ] }),
    /* @__PURE__ */ jsxs("div", { className: "p-4", children: [
      ...
      /* @__PURE__ */ jsxs("div", { className: "flex items-center gap-2 mt-3", children: [
        ...
        item.file_size && isImage && /* @__PURE__ */ jsx("span", { className: "text-xs text-gray-300", children: fmtMB(item.file_size) }),
...
function TabContent({
  ...
  return /* @__PURE__ */ jsxs("div", { className: "space-y-4", children: [
    canEdit && atQuota && /* @__PURE__ */ jsxs("div", { className: "flex items-start gap-2 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700", children: [
    ...
    canEdit && /* @__PURE__ */ jsx("div", { className: "flex justify-end", children: !showForm && !editItem && /* @__PURE__ */ jsxs("button", { type: "button", onClick: () => {
      if (atQuota) {
        toast.error("Storage limit reached for this member blog. Please delete old files or contact admin.");
        return;
      }
      setShowForm(true);
    }, className: "flex items-center gap-2 rounded-xl bg-amber-500 hover:bg-amber-600 text-white px-4 py-2 text-sm font-semibold transition-colors disabled:opacity-60", children: [
      /* @__PURE__ */ jsx(Plus, { className: "h-4 w-4" }),
      tab === "photos" ? "Add Photo" : tab === "videos" ? "Add Video" : tab === "files" ? "Upload File" : "Add Post"
    ] }) }),
    showForm && /* @__PURE__ */ jsx(ItemForm, { memberId, tab, usedBytes, onDone: () => setShowForm(false) }),
    editItem && /* @__PURE__ */ jsx(ItemForm, { memberId, tab, existing: editItem, usedBytes, onDone: () => setEditItem(null) }),
...
function MemberBlogPage() {
  ...
  return /* @__PURE__ */ jsxs("div", { className: "min-h-screen bg-gradient-to-b from-amber-50/60 to-white", children: [
    ...
    /* @__PURE__ */ jsxs("div", { className: "mx-auto max-w-5xl px-4 sm:px-6 py-8 space-y-6", children: [
      canEdit && storageUsage && /* @__PURE__ */ jsx(StorageBar, { usage: storageUsage }),
      activeTab === "about" ? /* @__PURE__ */ jsx(AboutTab, { member, canEdit }) : /* @__PURE__ */ jsx(TabContent, { memberId: member.id, tab: activeTab, canEdit, storageUsage })
    ] })
  ] });
}
function AboutTab({

import { c as createServerRpc } from "./createServerRpc-n2Wseq2T.js";
import { a as requireAdmin } from "./middleware-q9eKmX0S.js";
import { q as query, P as PROFILE_COLUMNS, w as withTransaction } from "../server.js";
import { c as createServerFn } from "./server-B57haCpF.js";
...
const adminEditEvent = createServerFn({
  ...
  if (data.url) {
    ...
    await query(`UPDATE events SET title=$2, description=$3, location=$4, event_date=$5, end_date=$6,
           ...
           updated_at=now()
         WHERE id=$1`, [data.id, title, description || null, location || null, data.event_date, data.end_date || null, data.event_type || "upcoming", data.rsvp_enabled ?? false, data.external_link || null, data.url, data.storagePath || null, data.fileName || null, data.fileSize ?? null, data.mimeType || null]);
  } else {
    await query(`UPDATE events SET title=$2, description=$3, location=$4, event_date=$5, end_date=$6,
           event_type=$7, rsvp_enabled=$8, external_link=$9, updated_at=now()
         WHERE id=$1`, [data.id, title, description || null, location || null, data.event_date, data.end_date || null, data.event_type || "upcoming", data.rsvp_enabled ?? false, data.external_link || null]);
  }
  return {
    ok: true
  };

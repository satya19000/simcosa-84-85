...
const adminAddAdminByEmail = createServerFn({
  ...
  const res = await query(`SELECT id FROM profiles WHERE email = $1`, [data.email.trim().toLowerCase()]);
  if (!res.rows[0]) {
    throw new Error("No member found with that email. They must sign up first.");
  }
  await query(`INSERT INTO user_roles (user_id, role) VALUES ($1, 'admin')
       ON CONFLICT (user_id, role) DO NOTHING`, [res.rows[0].id]);
  return {
    ok: true
  };
});
...
const adminListGallery = createServerFn({
  ...
  const res = await query(`SELECT g.id, g.title, g.caption, g.media_type, g.storage_path, g.created_at, g.uploaded_by,
         json_build_object('full_name', p.full_name) AS profiles

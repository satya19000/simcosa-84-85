import { c as createServerRpc } from "./createServerRpc-Bpua6jTj.js";
import { r as requireApproved, b as requireAdmin } from "./middleware-DxzaEMYp.js";
import { q as query } from "../server.js";
import { c as createServerFn } from "./server-DipaFqsh.js";
...
const ALLOWED_BLOG_IMAGE_TYPES = /* @__PURE__ */ new Set(["image/jpeg", "image/jpg", "image/png", "image/webp"]);
const LIST_COLUMNS = `
...
const listBlogs = createServerFn({
  method: "GET"
}).middleware([requireApproved]).inputValidator((d) => d).handler(listBlogs_createServerFn_handler, async ({
  data,
  context
}) => {
  const params = [];
...
const getBlog = createServerFn({
  method: "GET"
}).middleware([requireApproved]).inputValidator((d) => d).handler(getBlog_createServerFn_handler, async ({
  data,
  context
}) => {
...
const createBlog = createServerFn({
  method: "POST"
}).middleware([requireApproved]).inputValidator((d) => d).handler(createBlog_createServerFn_handler, async ({
  data,
  context
}) => {
  ...
  const res = await query(`INSERT INTO blogs (author_id, title, content, excerpt, category, image_url, fb_storage_path, file_name, file_size)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9) RETURNING id`, [context.userId, title, content, autoExcerpt, category, imageUrl, fbStoragePath, fileName, fileSize]);
  return {
    ok: true,
    id: res.rows[0].id
  };
...
const updateBlog = createServerFn({
  method: "POST"
}).middleware([requireApproved]).inputValidator((d) => d).handler(updateBlog_createServerFn_handler, async ({
  data,
  context
}) => {
...
const deleteBlog = createServerFn({
  method: "POST"
}).middleware([requireApproved]).inputValidator((d) => d).handler(deleteBlog_createServerFn_handler, async ({
  data,
  context
}) => {
  const owned = await query(`SELECT author_id, fb_storage_path FROM blogs WHERE id = $1`, [data.id]);
  const row = owned.rows[0];
  if (!row) throw new Error("Blog not found");
  if (row.author_id !== context.userId && !context.isAdmin) throw new Error("Forbidden");
...
const toggleBlogLike = createServerFn({
  method: "POST"
}).middleware([requireApproved]).inputValidator((d) => d).handler(toggleBlogLike_createServerFn_handler, async ({
  data,
  context
}) => {
  if (data.liked) {
...
const addBlogComment = createServerFn({
  method: "POST"
}).middleware([requireApproved]).inputValidator((d) => d).handler(addBlogComment_createServerFn_handler, async ({
  data,
  context
}) => {

import { c as createServerRpc } from "./createServerRpc-Bfb8LZfq.js";
...
const listEvents = createServerFn({
  method: "GET"
}).middleware([requireAuth]).handler(listEvents_createServerFn_handler, async () => {
...
const listMyRsvps = createServerFn({
  method: "GET"
}).middleware([requireAuth]).handler(listMyRsvps_createServerFn_handler, async ({
  context
}) => {
...
const listRsvpCounts = createServerFn({
  method: "GET"
}).middleware([requireAuth]).handler(listRsvpCounts_createServerFn_handler, async () => {
...
const setRsvp = createServerFn({
  method: "POST"
}).middleware([requireAuth]).inputValidator((d) => d).handler(setRsvp_createServerFn_handler, async ({
  data,
  context
}) => {

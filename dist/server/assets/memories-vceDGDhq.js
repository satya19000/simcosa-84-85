...
import { l as listMemories, p as postMemory, a as addMemoryImages, d as deleteMemory, b as deleteMemoryImage, r as reorderMemoryImages, t as toggleLike, e as editMemory, c as deleteComment, f as addComment } from "./memories-B0R6crRY.js";
import { u as useAuth, B as Button, a as cn } from "./router-Dnr0rR3U.js";
import { I as Input } from "./input-Sdk8Axix.js";
import { L as Label } from "./label-CqyKX9Z3.js";
import { T as Textarea } from "./textarea-B_jZaFEh.js";
import { Send, BookOpen, Pencil, Trash2, ImagePlus, ArrowUpDown, Heart, MessageCircle, X, ArrowLeft, ArrowRight } from "lucide-react";
import { format } from "date-fns";
import { toast } from "sonner";
import { I as ImageLightbox } from "./ImageLightbox-Dt94sEvv.js";
import { u as useUploadQueue, D as DropzoneUpload, d as deleteFromFirebaseStorage, c as compressImage, a as uploadToFirebaseStorageResumable } from "./useUploadQueue-BqJJ8p17.js";
import "./middleware-BuCFMbWx.js";
import "../server.js";
import "node:crypto";
import "pg";
import "jose";
import "./createMiddleware-BvN2ghIY.js";
import "./server-B6ODWh69.js";
...
async function uploadMemoryImages(files, userId, uploadQueue) {
  ...
  for (const original of files) {
    try {
      ...
      const res = await uploadToFirebaseStorageResumable(compressed, "memories", userId, (pct) => uploadQueue.setPct(original, pct));
      uploaded.push({
        url: res.url,
...
function MemoryImageGrid({
  ...
  if (images.length === 1 && !reordering) {
    ...
    return /* @__PURE__ */ jsxs("div", { className: "mt-4 relative overflow-hidden rounded-xl border border-amber-100", children: [
      /* @__PURE__ */ jsx("button", { type: "button", onClick: () => onOpen(0), className: "block w-full cursor-zoom-in", children: /* @__PURE__ */ jsx("img", { src: img.image_url, alt: "Memory photo", loading: "lazy", className: "w-full max-h-96 object-cover hover:scale-[1.02] transition-transform duration-300" }) }),
      canDelete && /* @__PURE__ */ jsx("button", { type: "button", "aria-label": "Remove photo", title: "Remove photo", onClick: () => onRemove(img.id), className: "absolute top-2 right-2 h-7 w-7 rounded-full bg-black/60 hover:bg-red-600 text-white flex items-center justify-center transition-colors", children: /* @__PURE__ */ jsx(X, { className: "h-3.5 w-3.5" }) })
    ] });
  }
  const gridCols = images.length === 2 ? "grid-cols-2" : "grid-cols-2 sm:grid-cols-3";
  return /* @__PURE__ */ jsx("div", { className: cn("mt-4 grid gap-1.5", gridCols), children: images.map((img, i) => /* @__PURE__ */ jsxs("div", { className: "relative overflow-hidden rounded-lg border border-amber-100 aspect-square", children: [
    /* @__PURE__ */ jsx("button", { type: "button", onClick: () => !reordering && onOpen(i), className: cn("h-full w-full block", !reordering && "cursor-zoom-in"), disabled: reordering, children: /* @__PURE__ */ jsx("img", { src: img.image_url, alt: "Memory photo", loading: "lazy", className: "h-full w-full object-cover hover:scale-[1.05] transition-transform duration-300" }) }),
    reordering ? /* @__PURE__ */ jsxs("div", { className: "absolute inset-0 bg-black/40 flex items-center justify-center gap-2", children: [
      /* @__PURE__ */ jsx("button", { type: "button", "aria-label": "Move photo earlier", disabled: i === 0, onClick: () => onMove?.(i, -1), className: "h-8 w-8 rounded-full bg-white/90 text-gray-800 flex items-center justify-center disabled:opacity-30 hover:bg-white", children: /* @__PURE__ */ jsx(ArrowLeft, { className: "h-4 w-4" }) }),
      /* @__PURE__ */ jsx("button", { type: "button", "aria-label": "Move photo later", disabled: i === images.length - 1, onClick: () => onMove?.(i, 1), className: "h-8 w-8 rounded-full bg-white/90 text-gray-800 flex items-center justify-center disabled:opacity-30 hover:bg-white", children: /* @__PURE__ */ jsx(ArrowRight, { className: "h-4 w-4" }) })
    ] }) : onRemove && !isLegacy(img) && /* @__PURE__ */ jsx("button", { type: "button", "aria-label": "Remove photo", title: "Remove photo", onClick: () => onRemove(img.id), className: "absolute top-1.5 right-1.5 h-6 w-6 rounded-full bg-black/60 hover:bg-red-600 text-white flex items-center justify-center transition-colors", children: /* @__PURE__ */ jsx(X, { className: "h-3.5 w-3.5" }) })
  ] }, img.id)) });
}
function Memories() {
  const {
  ...
  const [addingPhotosId, setAddingPhotosId] = useState(null);
  ...
  const handleMoveImage = async (memoryId, images, idx, dir) => {
  ...
  const lightboxImages = (lightboxMemory?.images ?? []).map((img) => ({
    src: img.image_url,
    ...
    /* @__PURE__ */ jsxs("div", { className: "mx-auto max-w-3xl px-4 sm:px-6 py-8", children: [
      /* @__PURE__ */ jsxs("form", { onSubmit: onPost, className: "bg-white rounded-2xl border border-amber-100 shadow-sm p-6 mb-8", children: [
        ...
        /* @__PURE__ */ jsxs("div", { className: "space-y-3", children: [
          ...
          /* @__PURE__ */ jsxs("div", { children: [
            /* @__PURE__ */ jsx(Label, { className: "font-semibold text-gray-700", children: "Add photos (optional)" }),
            /* @__PURE__ */ jsx("p", { className: "text-xs text-gray-400 mt-0.5 mb-1", children: "You can select or drag multiple photos for the same memory." }),
            /* @__PURE__ */ jsx(DropzoneUpload, { files: photoFiles, onFilesChange: setPhotoFiles, accept: "image/*", multiple: true, disabled: posting, className: "mt-1", progress: uploadQueue.progress })
          ] })
        ] }),
          ...
          return /* @__PURE__ */ jsxs("article", { className: "bg-white rounded-2xl border border-amber-100 shadow-sm overflow-hidden", children: [
            /* @__PURE__ */ jsxs("div", { className: "p-6", children: [
              ...
              editingId === m.id ? /* @__PURE__ */ jsx(EditMemoryPanel, { memory: m, onDone: () => setEditingId(null) }) : /* @__PURE__ */ jsxs(Fragment, { children: [
                ...
                /* @__PURE__ */ jsx(MemoryImageGrid, { images: m.images, onOpen: (idx) => setLightbox({
                  memoryId: m.id,
                  index: idx
                }), reordering: reorderingId === m.id, onMove: (idx, dir) => handleMoveImage(m.id, m.images, idx, dir), memoryId: m.id, onRemove: canManage ? handleRemoveImage : void 0 }),
                canManage && /* @__PURE__ */ jsxs("div", { className: "mt-3 flex flex-wrap items-center gap-4", children: [
                  addingPhotosId !== m.id && /* @__PURE__ */ jsxs("button", { type: "button", onClick: () => setAddingPhotosId(m.id), className: "flex items-center gap-1.5 text-sm font-semibold text-amber-600 hover:text-amber-700", children: [
                  ...
                  m.images.length > 1 && /* @__PURE__ */ jsxs("button", { type: "button", onClick: () => setReorderingId(reorderingId === m.id ? null : m.id), className: "flex items-center gap-1.5 text-sm font-semibold text-amber-600 hover:text-amber-700", children: [
                    ...
                    " ",
                    reorderingId === m.id ? "Done reordering" : "Reorder photos"
                  ] })
                ] }),
                canManage && addingPhotosId === m.id && /* @__PURE__ */ jsx(AddPhotosPanel, { memoryId: m.id, existingImages: m.images, onDone: () => setAddingPhotosId(null) })
              ] })
            ] }),
...
function AddPhotosPanel({
  ...
  return /* @__PURE__ */ jsxs("div", { className: "mt-3 space-y-2 bg-amber-50/50 rounded-xl p-3", children: [
    /* @__PURE__ */ jsx(DropzoneUpload, { files, onFilesChange: setFiles, accept: "image/*", multiple: true, disabled: saving, progress: uploadQueue.progress }),
...
function EditMemoryPanel({
  ...
  return /* @__PURE__ */ jsxs("div", { className: "space-y-3 bg-amber-50/50 rounded-xl p-4", children: [
    ...
    canManageImages && memory.images.length > 0 && /* @__PURE__ */ jsx("div", { className: "grid grid-cols-3 sm:grid-cols-4 gap-2", children: memory.images.map((img) => /* @__PURE__ */ jsxs("div", { className: "relative rounded-lg overflow-hidden border border-amber-100 aspect-square", children: [
      /* @__PURE__ */ jsx("img", { src: img.image_url, alt: "", className: "h-full w-full object-cover" }),
      /* @__PURE__ */ jsx("button", { type: "button", onClick: () => removeImage(img.id), "aria-label": "Remove photo", className: "absolute top-1 right-1 h-6 w-6 rounded-full bg-black/60 hover:bg-red-600 text-white flex items-center justify-center", children: /* @__PURE__ */ jsx(X, { className: "h-3.5 w-3.5" }) })
    ] }, img.id)) }),
    canManageImages && /* @__PURE__ */ jsxs("div", { children: [
      ...
      /* @__PURE__ */ jsx(DropzoneUpload, { files: newFiles, onFilesChange: setNewFiles, accept: "image/*", multiple: true, disabled: saving, progress: uploadQueue.progress, className: "mt-1" })
    ] }),
    /* @__PURE__ */ jsxs("div", { className: "flex items-center justify-end gap-2", children: [

  ...
  const {
    ...
    queryFn: async () => {
      const {
        data,
        error
      } = await supabase.from("event_rsvps").select("*").eq("user_id", user.id);
      if (error) throw error;
      return data;
    }
  });
  const {
    ...
    queryFn: async () => {
      const {
        data,
        error
      } = await supabase.from("event_rsvps").select("event_id,status");
      if (error) throw error;
      const c = {};
      for (const r of data) {
        c[r.event_id] ??= {
          ...
          return /* @__PURE__ */ jsxs("div", { className: `bg-white rounded-2xl border shadow-sm overflow-hidden ${isNext ? "border-amber-400 shadow-amber-100" : "border-amber-100"}`, children: [
            ...
            /* @__PURE__ */ jsxs("div", { className: "p-6", children: [
              ...
              /* @__PURE__ */ jsx("div", { className: "mt-5 flex flex-wrap gap-2", children: RSVP_OPTIONS.map(({
                ...
                icon: Icon,
                cls
              }) => /* @__PURE__ */ jsxs(Button, { onClick: () => setRsvp(e.id, value), className: `h-11 px-5 rounded-xl font-bold text-sm flex items-center gap-2 transition-all ${my === value ? cls : "bg-white border border-gray-200 text-gray-600 hover:border-amber-400 hover:text-amber-700"}`, children: [

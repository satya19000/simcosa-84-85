...
import { r as requireApproved } from "./middleware-BymfKMq5.js";
...
const ALLOWED_ATTACHMENT_TYPES = /* @__PURE__ */ new Set(["image/jpeg", "image/jpg", "image/png", "image/webp", "image/gif", "video/mp4", "video/quicktime", "video/webm", "application/pdf", "application/msword", "application/vnd.openxmlformats-officedocument.wordprocessingml.document", "application/vnd.ms-powerpoint", "application/vnd.openxmlformats-officedocument.presentationml.presentation", "application/vnd.ms-excel", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "text/plain", "text/csv", "application/zip", "application/x-zip-compressed"]);
function deriveAttachmentType(mimeType) {
...
const addMemoryImages = createServerFn({
  ...
  const maxOrdRes = await query(`SELECT MAX(sort_order) AS max FROM memory_images WHERE memory_id = $1`, [data.memoryId]);
  ...
  for (const img of data.images) {
    ...
    if (img.fileName && img.fileSize) {
      const dup = await query(`SELECT id FROM memory_images WHERE memory_id = $1 AND file_name = $2 AND file_size = $3 LIMIT 1`, [data.memoryId, img.fileName, img.fileSize]);
      if (dup.rows.length > 0) continue;
    }
    const attachmentType = img.attachmentType || deriveAttachmentType(img.mimeType || "");
    await query(`INSERT INTO memory_images (memory_id, image_url, fb_storage_path, file_name, mime_type, file_size, sort_order, attachment_type)
         VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`, [data.memoryId, img.url, img.storagePath || null, img.fileName || null, img.mimeType || null, img.fileSize ?? null, nextOrder++, attachmentType]);
  }
  return {
    ok: true
  };
...
const reorderMemoryImages = createServerFn({
  ...
  if (data.orderedImageIds.length !== existingIds.size || !data.orderedImageIds.every((id) => existingIds.has(id))) {
    throw new Error("Attachment list is out of date. Please refresh and try again.");
  }
